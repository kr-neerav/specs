...
7. **Quarantined Context Stripping**: To maintain the epistemic chain of custody and consensus logic across the multi-agent system, the orchestrator MUST NOT strip the `thought_log` reasoning scratchpad fields from upstream JSON payloads when passing them to downstream agents (Personas 1 through 4). All agents share reasoning. The `thought_log` may only be stripped or hidden at the UI rendering boundary.
8. **Memory Vacuum Safeguard**: (Deprecated) Upstream `thought_log` reasoning logs are no longer stripped.
